...
class GPUObject
{
  public:
	...
	{
		LOG_WARN("GPUObject copy constructor called - cloning GPU handles is "
		         "not safe!");
		m_ID = 0;
	}
	...
	{
		LOG_WARN("GPUObject copy assignment called - cloning GPU handles is "
		         "not safe!");
		if (this != &other)
		{
			Unload();
	...
	virtual void Unload()
	{
		...
		{
			LOG_WARN(
			    "Base GPUObject::Unload() called - should be overridden!\n");
			m_ID = 0;
		}
	}
